...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  1 17:14:11 2018
...
def _self_bahdanau_score(keys, normalize):
  """Implements Bahdanau-style (additive) scoring function.
  This attention has two forms.  The first is Bhandanau attention,
  as described in:
  Dzmitry Bahdanau, Kyunghyun Cho, Yoshua Bengio.
...
class SelfBahdanauAttention(_BaseAttentionMechanism):
  """Implements Bahdanau-style (additive) attention.
  This attention has two forms.  The first is Bahdanau attention,
  as described in:
  Dzmitry Bahdanau, Kyunghyun Cho, Yoshua Bengio.
  ...
  def __init__(self,
    ...
    if dtype is None:
      dtype = dtypes.float32
    wrapped_probability_fn = lambda score, _: probability_fn(score)
    super(SelfBahdanauAttention, self).__init__(
        query_layer=layers_core.Dense(
            num_units, name="query_layer", use_bias=False, dtype=dtype),
        memory_layer=layers_core.Dense(
            num_units, name="memory_layer", use_bias=True, dtype=dtype),
        memory=memory,
        probability_fn=wrapped_probability_fn,
        memory_sequence_length=memory_sequence_length,
        score_mask_value=score_mask_value,
  ...
  def __call__(self, previous_alignments):
    """Score the query based on the keys and values.
    Args:
      query: Tensor of dtype matching `self.values` and shape
        `[batch_size, query_depth]`.
    ...
    Returns:
      alignments: Tensor of dtype matching `self.values` and shape
        ...
        `max_time`).
    """
    with variable_scope.variable_scope(None, "self_bahdanau_attention"):
      # processed_query = self.query_layer(query) if self.query_layer else query
      score = _self_bahdanau_score(self._keys, self._normalize)
    alignments = self._probability_fn(score, previous_alignments)
    
    output = tf.reduce_sum(self.memory * tf.expand_dims(alignments, -1), 1)
