...
# -*- coding: utf-8 -*-
"""
Created on Sat May 20 15:40:23 2017
...
class Solution(object):
    def eraseOverlapIntervals(self, intervals):
        ...
        if len(intervals) <= 1:
            return 0
        #inters = intervals # for the following test, take the place of the next 3 lines
        inters = []
        for item in intervals:
            inters.append([item.start,item.end])
        inters.sort(key=lambda x:x[1])
        inters.sort(key=lambda x:x[0])
        previous = inters[0]
        result = 0
        for i in xrange(1,len(inters)):
            if inters[i][0] < previous[1]:
                previous[1] = min(previous[1],inters[i][1])
                result += 1
            else:
                previous = inters[i]
        return result
